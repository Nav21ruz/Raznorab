  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-wrap gap-2">
        {photos.map((url, idx) => (
          <div key={url} className="relative w-20 h-20 rounded-xl overflow-hidden border border-border-2 shrink-0">
        ...
        {photos.length < max && (
          <button
            ...
            className="w-20 h-20 rounded-xl border border-dashed border-border-2 flex items-center justify-center text-text-muted hover:border-copper-500/40 hover:text-text-muted transition-colors shrink-0 disabled:opacity-50"
          >
            {upload.isPending ? (
              <span className="w-4 h-4 border-2 border-border-2 border-t-copper-500 rounded-full animate-spin" />
            ) : (
