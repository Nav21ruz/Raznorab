  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-text-primary">Разнорабочие</h1>
          <p className="text-sm text-text-muted mt-1">{tasks?.length ? `${tasks.length} задач(и)` : 'Опубликуйте разовую задачу'}</p>
        </div>
      ...
      {!isLoading && tasks?.length === 0 && (
        <div className="text-center py-24">
          ...
          <p className="text-text-secondary font-medium">Нет задач</p>
          <p className="text-sm text-text-muted mt-1">Нужен разнорабочий на разовую подработку? Опубликуйте задачу</p>
        </div>
      )}
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-1.5">
        ...
        <span className={`shrink-0 px-2 py-1 rounded-lg text-xs font-medium ${task.status === 'active' ? 'bg-success-bg text-success-text' : 'bg-border-2/50 text-text-secondary'}`}>
      ...
      <div className="flex flex-wrap gap-3 text-xs text-text-muted">
        ...
        {!!responses?.length && (
          <span className="flex items-center gap-1 text-copper-hover"><Users2 className="w-3.5 h-3.5" />{responses.length} отклик(ов)</span>
        )}
      </div>
    </button>
  )
